...
Netzke.pre.GridPanel = Ext.extend(Ext.grid.EditorGridPanel, {
 ...
 listeners: {
     beforeedit : function(e) {
         ...
         console.log(e);
         Netzke.pre.GridPanel.currentlyEditedRecord = e.record;
         Netzke.pre.GridPanel.currentlyEditedField = e.field;
...
//                
    ...
    this.store = new Ext.data.Store({
		...
		update     : function(store, record, operation) {
					alert('update fired');
		            switch(Netzke.pre.GridPanel.currentlyEditedField){
		                case "amount":
		                    alert('trying to exit the amount');
		                    break;
		                case "Field2":
		                    break;
		            }
